...
Structures large initiatives into a story backlog with a roadmap, dependencies, and verification criteria. Use when work requires several coordinated stories, has cross-dependencies between deliveries, or needs a phased roadmap to guide execution.

## When to use

- After a `/agile-refinement` that generated several stories that need coordination
- An initiative is large, requiring multiple stories, and can't be a single story
...
- The work fits in a single story — use `/agile-story` instead
- The work is small and localized — use `/agile-task-plan` instead
...
5. The skill produces `planning/payment-system-overhaul/epic.md` with:
   ...
   - Story 3: Payout reconciliation (medium, depends on 1) — Phase 2
   - Story 4: Invoice migration (large, depends on 1, 2) — Phase 2
...
- The epic organizes and sequences stories — it does not replace them. Each story still needs its own `/agile-story` or `/agile-task-plan`.
...
- **After:** `/agile-story` (detail individual stories), `/agile-task-plan` (create execution plans for stories)
