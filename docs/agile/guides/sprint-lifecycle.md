...
**Skills covered:** ceremonies-router, sprint-planning, daily, sprint-review, sprint-metrics, retro
...
**Analysis:**
- *What worked:* TDD caught a regression early; dailies surfaced blockers within 24h
...
```
/sprint-planning week of April 14
```
...
### Daily checkpoints (Tue-Thu)

```
/daily
```

Quick inline dailies — no file saved for a solo cycle:
...
**Recommendation:** "Start with `/refinement` to clarify scope and size items. Then `/sprint-planning` to define the first sprint."
...
```mermaid
flowchart LR
    A[sprint-metrics] --> B[sprint-review]
    B --> C[retro]
    C --> D[sprint-planning]
    D --> E[daily x N]
    E --> F[post-impl]
...
|----------|------|-----------------|-------|
| Sprint Metrics | End of sprint, before review | Quantitative data for review and retro | `/sprint-metrics` |
| Sprint Review | End of sprint, after metrics | Delivery demo + stakeholder feedback | `/sprint-review` |
| Retrospective | After review, before planning | 2-3 improvement actions with owners | `/retro` |
| Sprint Planning | Start of new sprint | Sprint objective + selected items + order | `/sprint-planning` |
| Daily | Every day during sprint | Progress, blockers, next step | `/daily` |
| Post-impl | When a delivery closes | Formal closure with verification | `/post-impl` |

**Not sure which ceremony?** Use `/ceremonies-router` — it asks where you are in the cycle and points to the right one.
